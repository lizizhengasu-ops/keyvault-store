...
// ========== Button Size Constraints (prevents oversized buttons) ==========
add_action("wp_head", function() {
    echo '<style>
    /* Constrain WordPress block buttons (Browse/Buy on homepage) */
    ...
    /* Browse category buttons on homepage */
...
// ========== ULTIMATE BUTTON FIX - JS runtime override ==========
add_action("wp_footer", function() {
    echo '<script>
    document.addEventListener("DOMContentLoaded", function() {
        var btns = document.querySelectorAll(".wp-block-button__link");
        for (var i = 0; i < btns.length; i++) {
            btns[i].style.setProperty("padding", "4px 12px", "important");
            btns[i].style.setProperty("font-size", "11px", "important");
            btns[i].style.setProperty("min-height", "auto", "important");
            btns[i].style.setProperty("line-height", "1.2", "important");
            btns[i].style.setProperty("display", "inline-block", "important");
            btns[i].style.setProperty("width", "auto", "important");
        }
    });
    </script>';
}, 100);
...
// ========== EMERGENCY BUTTON CSS - wp_head ==========
add_action("wp_head", function() {
    echo '<style id="emergency-btn-fix">
    ...
    .wp-block-button__link,.add_to_cart_button,a[class*=button]{padding:6px 14px!important;font-size:12px!important;min-height:auto!important;line-height:1.3!important;white-space:nowrap!important}
    </style>';
}, 1);
?>
...
add_action("wp_enqueue_scripts", function() {
    if (is_page(162)) {
        ...
        wp_enqueue_script("test-page-anim", get_stylesheet_directory_uri() . "/assets/js/test-page-anim.js", array("gsap"), "1.0", true);
        wp_enqueue_style("font-awesome", "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css");
    }
});
// ========== Button Size Constraints - wp_footer (loads AFTER block CSS) ==========
add_action("wp_footer", function() {
    echo '<style id="keys-btn-fix">
