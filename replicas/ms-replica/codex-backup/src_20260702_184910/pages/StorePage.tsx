export default function StorePage() {
  ...
  return (
    <div style={{maxWidth:1200,margin:'0 auto',padding:'80px 24px'}}>
      <h1 style={{fontSize:46,fontWeight:600,lineHeight:1.1,marginBottom:8}}>Store</h1>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
        {products.map((p,i)=>(
          <div key={i} style={{background:'#f2f2f2',padding:'32px',borderRadius:2,cursor:'pointer'}}>
            <div style={{width:'100%',height:160,background:p.img,borderRadius:2,marginBottom:16,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:24,fontWeight:600}}>{p.t[0]}</div>
