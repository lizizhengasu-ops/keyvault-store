export default function StorePage() {
  ...
  const explore = ["Trade in and save","Why buy from Microsoft Store","Frequently asked questions","Student discounts","Military discounts","Business pricing"]
  return (
    <div style={{fontFamily:'"Segoe UI Variable Text","Segoe UI",system-ui,sans-serif',color:'#000',background:'#fff',padding:"48px 48px",maxWidth:1200,margin:"0 auto"}}>
      <h1 style={{fontSize:32,fontWeight:700,lineHeight:"38px",color:'#000',margin:"0 0 16px 0"}}>Microsoft Store Deals</h1>
      <p style={{fontSize:20,fontWeight:400,lineHeight:"24px",color:'#000',margin:"0 0 32px 0"}}>Save big on Surface, Xbox, and more.</p>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>
        {features.map((f,i)=>(
          <div key={i} style={{padding:16,background:"#fff",border:"1px solid #e6e6e6",borderRadius:2}}>
      ...
      <h2 style={{fontSize:24,fontWeight:700,color:"#0e1726",lineHeight:"28px",margin:"48px 0 24px 0"}}>Explore Microsoft</h2>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>
        {explore.map((e,i)=>(
          <div key={i} style={{padding:16,background:"#fff",border:"1px solid #e6e6e6",borderRadius:2}}>
            <a href="#" style={{fontSize:13,color:"#0078D4",fontWeight:400,textDecoration:"none"}}>{e}</a>
          </div>
        ))}
      </div>
