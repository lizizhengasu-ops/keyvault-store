        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
          {topics.map((t,i)=>(
            <div key={i} style={{background:"rgb(242,242,242)",padding:20,borderRadius:2}}>
              ...
              <a href="#" style={{fontSize:12,color:"#363636",textDecoration:"none",display:"block",marginTop:4}}>Get help</a>
            </div>
          ))}
        </div>
      </section>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
            {articles.slice(0,4).map((a,i)=>(
              <div key={i} style={{background:"#fff",padding:16,borderRadius:2}}>
                ...
                <a href="#" style={{fontSize:12,color:"#363636",textDecoration:"none"}}>Learn more</a>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
          {articles.slice(4,8).map((a,i)=>(
            <div key={i} style={{background:"rgb(242,242,242)",padding:16,borderRadius:2}}>
              ...
              <div style={{fontSize:12,color:"rgb(97,97,97)",marginBottom:4}}>{a.cat}</div>
              <h3 style={{fontSize:15,fontWeight:600,color:"#242424",margin:"0 0 4px 0",lineHeight:"20px"}}>{a.title}</h3>
              <a href="#" style={{fontSize:12,color:"#363636",textDecoration:"none"}}>Get started</a>
            </div>
          ))}
        </div>
      </section>
      <section style={{background:"rgb(242,242,242)",padding:"48px 48px"}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:24}}>
            {resources.map((r,i)=>(<a key={i} href="#" style={{fontSize:13,color:"#363636",textDecoration:"none",padding:"4px 12px",background:"#fff",borderRadius:2}}>{r}</a>))}
          </div>
        </div>
      </section>
      <div style={{display:"flex",flexWrap:"wrap",gap:2,padding:"24px 48px",justifyContent:"center"}}>
        {flinks.map((l,i)=>(<a key={i} href="#" style={{fontSize:11,color:"#616161",lineHeight:"16px",textDecoration:"none",padding:"2px 4px"}}>{l}</a>))}
      </div>
    </div>
  )
}