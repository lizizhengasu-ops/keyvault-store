...
const items = [
  ...
  {n:'mPhone 4',p:399,s:'mphone-4',c:'#1c1c1e',d:'5MP LED',disp:'3.5in Retina',chip:'A4',badge:''}
];
  ...
  return (
    <>
      <section style={{padding:'48px 20px 0',maxWidth:1068,margin:'0 auto'}}>
        ...
        <p style={{fontSize:21,fontWeight:300,color:'#1d1d1f',marginBottom:40,lineHeight:1.19048,letterSpacing:'.011em'}}>The best way to buy the products you love.</p>
      </section>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          {items.slice(0,3).map(x => (
            <Link to={'/product/'+x.s} key={x.s} className='card-hover' style={{display:'flex',gap:0,background:'#f5f5f7',borderRadius:18,overflow:'hidden',textDecoration:'none',color:'inherit',border:'1px solid #e6e6ea'}}>
              <div style={{width:130,minHeight:130,display:'flex',alignItems:'center',justifyContent:'center',background:'#f5f5f7',padding:16}}>
              ...
              <div style={{flex:1,padding:'20px 20px 20px 0',display:'flex',flexDirection:'column',justifyContent:'center'}}>
                ...
                <p style={{fontSize:17,color:'#1d1d1f',fontWeight:400}}>From ${x.p}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section style={{padding:'40px 20px',background:'#f5f5f7'}}>
        <h2 style={{fontSize:34,fontWeight:600,lineHeight:1.47059,textAlign:'center',marginBottom:24,color:'#1d1d1f'}}>Why shop at mifan?</h2>
        <div className='stagger-grid' style={{maxWidth:900,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16}}>
          {[
            {t:'Free Delivery',d:'Free no-contact delivery on all mPhone orders. Choose a delivery window that works for you.',icon:'**'},
            {t:'0% Financing',d:'Pay over time with 0% APR financing. As low as $41.62/mo. No interest. No hidden fees.',icon:'**'},
            {t:'Trade In',d:'Get $200-$650 credit toward a new mPhone. It is good for you and the planet.',icon:'**'},
            {t:'Personal Setup',d:'Get help setting up your new device online or in-store. We will transfer your data.',icon:'**'},
            {t:'24/7 Support',d:'Expert support whenever you need it. Call, chat, or visit us online. Always available.',icon:'**'},
            {t:'Free Returns',d:'Not loving it? Return within 14 days for a full refund. No restocking fees.',icon:'**'}
          ].map((f,i) => (
            <div key={i} className='stagger-item card-hover' style={{background:'#fff',borderRadius:18,padding:'24px',border:'1px solid #e6e6ea'}}>
      ...
      <section style={{padding:'40px 20px',background:'#fff'}}>
        <h2 style={{fontSize:34,fontWeight:600,lineHeight:1.47059,textAlign:'center',marginBottom:20,color:'#1d1d1f'}}>Compare all mPhones</h2>
