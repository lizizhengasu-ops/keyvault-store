          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>
            {deals.slice(0,3).map((d,i)=>(
              <div key={i} style={{background:"rgba(255,255,255,0.08)",padding:20,borderRadius:2,border:"1px solid rgba(255,255,255,0.12)"}}>
                ...
                <Link to="/products" style={{fontSize:13,color:"#0078D4",textDecoration:"none"}}>Shop now</Link>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
          {categories.map((c,i)=>(
            <div key={i} style={{background:"rgb(242,242,242)",padding:20,borderRadius:2}}>
              ...
              <Link to="/products" style={{fontSize:13,color:"#0067b8",display:"block",marginTop:8,textDecoration:"none"}}>Shop all {c.name} &gt;</Link>
            </div>
          ))}
        </div>
      </div>
