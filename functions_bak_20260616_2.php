...
add_action("woocommerce_before_main_content", function() {
    if (!is_shop() && !is_product_category() && !is_product_tag()) return;
    echo '<div class="ms-hero" style="position:relative;overflow:hidden;border-radius:8px;margin:0 0 24px 0;background:linear-gradient(135deg,#0078d4,#004578);padding:40px;text-align:center;">';
    echo '<h2 style="font-size:28px;color:#fff;margin:0 0 8px 0;font-weight:600;">Windows & Office Keys</h2>';
    echo '<p style="font-size:16px;color:rgba(255,255,255,0.9);margin:0 0 20px 0;">Genuine licenses at unbeatable prices. Instant delivery guaranteed.</p>';
    echo '<a href="/shop/" style="display:inline-block;background:#fff;color:#0078d4;padding:10px 28px;border-radius:4px;text-decoration:none;font-weight:600;font-size:15px;">Shop All Products</a>';
    echo '</div>';
}, 1);
...
add_action("wp_head", function() {
    ...
    echo '<style id="ms-lang-css">.ms-lang-top{position:fixed;top:6px;right:120px;z-index:99999}.ms-lang-top select{padding:2px 4px;border:1px solid #e6e6e6;border-radius:3px;font-size:11px;background:#fff;cursor:pointer}</style>';
    echo '<div class="ms-lang-top">';
    pll_the_languages(array("dropdown" => 1, "show_flags" => 0, "show_names" => 1));
    echo '</div>';
}, 1);
