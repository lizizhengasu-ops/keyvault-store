...
// ========== Language Switcher (JS in wp_footer) ==========
add_action('wp_footer', 'ms_lang_switcher_js');
function ms_lang_switcher_js() {
    ?>
<script>
(function(){
    var sel = document.createElement('select');
    sel.id = 'ms-lang-sel';
    sel.style.cssText = 'margin:8px 16px;padding:4px 8px;border:1px solid #e6e6e6;border-radius:4px;font-size:12px;background:#fff;cursor:pointer;';
    sel.onchange = function(){ if(this.value) window.location.href = this.value; };
    var langs = [
        {url:'/',name:'English',current: window.location.pathname==='/' || window.location.pathname.indexOf('/en/')===0},
        {url:'/zh/',name:'中文',current: window.location.pathname.indexOf('/zh/')===0},
        {url:'/es/',name:'Español',current: window.location.pathname.indexOf('/es/')===0}
    ];
    ...
    var nav = document.querySelector('.main-header-bar-navigation') || document.querySelector('.main-navigation ul') || document.querySelector('header .ast-main-header-bar-alignment');
    if(nav){
